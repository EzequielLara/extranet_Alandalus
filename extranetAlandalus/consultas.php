    ...
    while($i<count($fila)){
      ...
      if($i == count($fila)&&($fila[$numeroColumna]!=$valor)){
        ...
        //Al pulsar botón redirecionamos la variable que identifica al alumno para poder realizar los cambios oportunos en la BBDD
         echo '<td><a href="controlSesiones.php?datos=alumnos&nombre=Amador&perfil=PROFESOR&id_Alumno='.$fila[0].'&apellidos='.$fila[4].'&nombre_Alumno='.$fila[3].'"><button type="button" class="btn btn-warning">Baja</button></a></td>';
